...
u32_t begin_calc_crc32() {
    return -1;
}

u32_t continue_calc_crc32(u32_t remainder, const void* data, size_t length) {
    const u8_t* data_byte = (const u8_t*)data;
...
u32_t end_calc_crc32(u32_t remaider) {
    return reflect32(~remaider);
}
