...
DEF_SYSCALL(mmap, ptr, length, prot, flags, fd, offset) {
    ...
    if(prot == 0) {
        ...
        TRACE("Mapped %d bytes starting at 0x%x16 as NULL\n", page_len << 12, addr);
        return addr;
    }
    ...
    if(flags & MMAP_FLAG_ANONYMOUS) {
        ...
        TRACE("Mapped %d bytes starting at 0x%x16 as ANON\n", page_len << 12, addr);
        return addr;
    }
    ...
    TRACE("Mapped %d bytes starting at 0x%x16 to FILE with offset %d\n", page_len << 12, addr, offset);
...
DEF_SYSCALL(munmap, ptr, length) {
    ...
    TRACE("Unmapped %d bytes starting at 0x%x16\n", page_len << 12, ptr);
    return 0;
}
