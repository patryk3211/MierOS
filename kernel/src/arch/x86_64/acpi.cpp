...
extern "C" TEXT_FREE_AFTER_INIT void init_acpi(physaddr_t rsdp) {
    ...
    if(check2 == 0) {
        ...
        xsdt = (ACPI_XSDT*)pager.kmap(xsdt_addr, page_size, { 1, 0, 0, 0, 0, 0 });
        ...
        for(size_t i = 0; i < entries; ++i) {
            ...
            dmesg("(Kernel) ACPI Table (Signature '%s') found at 0x%016x", sign, addr);
        }
        ...
        rsdt = (ACPI_RSDT*)pager.kmap(rsdt_addr, page_size, { 1, 0, 0, 0, 0, 0 });
        ...
        for(size_t i = 0; i < entries; ++i) {
            ...
            dmesg("(Kernel) ACPI Table (Signature '%s') found at 0x%08x", sign, addr);
        }
